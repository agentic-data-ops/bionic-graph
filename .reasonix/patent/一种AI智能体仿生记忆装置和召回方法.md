...
（ASCII 示意：一条记忆及索引的登记关系）
  token ──倒排──▶ [记忆单元A]──边:爱好(strength=0.9)──▶[记忆单元B]
                     │  ▲                                  │
...
````
记忆单元 A：name="李先生"，labels=["用户"]，keywords=["摄影","常去公园"]
记忆单元 B：name="摄影"，labels=["爱好"]，keywords=["相机","拍照"]
关联单元 E：source=A，target=B，name="爱好"，strength=0.9
````

写入时执行分词，将"李先生""摄影""公园""相机"等 token 写入倒排索引；初始化 `rank=0`、`atime=now`，写入频度索引与时间索引。所有单元携带 `version=0`、`updated_at=now` 并初始化历史快照链。
...
- 2025-01-01 智能体记录项目决策"采用微服务架构"（记忆单元 B）；2025-02-15 更新 B（补充决策理由"团队扩容"）；2025-03-01 智能体删除记忆单元 B（该架构决策废弃）；2025-06-01 智能体新增记忆单元 E（"新架构迁移计划"）。
- 在时间戳 t=2025-02-01 执行时间旅行召回"架构决策相关记忆"：B 在 t 时刻尚未被删除 → **可见**（返回 v0 快照"采用微服务架构"）；E 的 `created_at` 晚于 t → **不可见**。智能体回答"我在 2 月 1 日还认为采用微服务架构，且不知道新架构迁移计划"。
- 在 t=2025-07-01 执行同样查询：B 已删除 → 不可见；E 可见。
- 若在 t=2025-02-20 查询，B 返回更新后的 v1 快照（"采用微服务架构，理由：团队扩容"）；在 t=2025-03-01 之后查询，B 已删除——实现了"任意时刻的记忆快照回溯"。
...
经两轮联网逐条筛选，检索到与本申请保护点相关的代表性专利如下（按相关度排列），均存在实质性差异：
