...
def load_json_to_graph(client: Client, graph_name: str, data: dict) -> dict[str, Any]:
    ...
    Entities are deduped by 'name'. Relations use string source/target
    (entity names) that are resolved to integer vertex IDs.
    ...
    for entity in data.get("entities", []):
        ...
        if ename in name_to_id:
            ...
            name_to_id[ename] = resp.id
            print(f"  ✅ Created '{ename}' (id={resp.id})")
    ...
    for rel in data.get("relations", []):
        ...
        print(f"  🔗 Created edge '{rel_name}'  {src_name} → {tgt_name}  (id={resp.id})")
